...
public class RegistrarApiController implements RegistrarApi {
		...
		if (accept != null && accept.contains("application/json")) {
			try {
				if (body.getPersona().get(0).getNombre().isEmpty()||
						...
						body.getPersona().get(0).getContrasena().isEmpty()) {
					error.setCodigo("011");
					error.setDetalle("verifique que todos los campos estan llenos");
				...
				if (!valid.validarContrasena(body.getPersona().get(0).getContrasena())) {
					error.setCodigo("012");
					error.setDetalle("la contraseña con cumple con el formato");
					return new ResponseEntity<JsonApiBodyResponseErrors>(error, HttpStatus.BAD_REQUEST);
				}
				...
				if (body.getPersona().get(0).getRol().equals("super administrador")) {
					...
					if (personas.get(0).getRol().equals("super administrador")) {
						...
						System.out.println("´´´´´´´´´´´´´´´´´´´´´´´´´´´´"+body.getPersona().get(0).getApellidos());
	...
	public ResponseEntity<?> loginPost(@ApiParam(value = "body", required = true) @Valid @RequestBody JsonApiBodyRequestLogin body) {
		try {
			if (body.getPersona().get(0).getCorreo().isEmpty()||body.getPersona().get(0).getContrasena().isEmpty()) {
				error.setCodigo("012");
				error.setDetalle("los campos no pueden estar vacios");
				return new ResponseEntity<JsonApiBodyResponseErrors>(error, HttpStatus.FORBIDDEN);
			}
			personas = persona_repository.findBycorreo(body.getPersona().get(0).getCorreo());
			if (personas.isEmpty()) {
				error.setCodigo("013");
				error.setDetalle("no se encontro personas");
				return new ResponseEntity<JsonApiBodyResponseErrors>(error, HttpStatus.FORBIDDEN);
			}
			if (personas.get(0).getContrasena().equals(body.getPersona().get(0).getContrasena())) {

				JsonApiBodyRequest body2 = new JsonApiBodyRequest();
				List<RegistrarRequest> lista = new ArrayList<RegistrarRequest>();
				lista.add(personas.get(0));
				body2.setPersona(lista);
				return new ResponseEntity<JsonApiBodyRequest>(body2,HttpStatus.OK);
			}
			error.setCodigo("014");
			error.setDetalle("correo o contraseña incorrecta");
			return new ResponseEntity<JsonApiBodyResponseErrors>(error,HttpStatus.BAD_REQUEST);
